...
case class getCoordinates(latitude: Double, longitude: Double) extends YahooWeatherService

object YahooWeatherService extends Logger {
  @throws(classOf[Exception])
  def apiCall(coordinates: JsonObject): String = {
    val url = (decorateEndpointUrl(getCoordinates.apply(coordinates.get("lat").getAsDouble(), coordinates.get("lon").getAsDouble())))
    RestClient.Call(url)
  }

  def decorateEndpointUrl(coordinates: getCoordinates): String =
